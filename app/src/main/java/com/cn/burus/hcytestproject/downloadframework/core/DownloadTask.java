...
import com.cn.burus.hcytestproject.downloadframework.utilities.TickTack;
...
public class DownloadTask implements ConnectThread.ConnectListener, DownloadThread.DownloadListener {
    ...
    public void start() {
        if (entry.totalLength > 0) {
            KLog.e("no need to check if support range and totalLength");
            startDownload();
        } else {
...
//        entry.totalLength = -1;
        KLog.e("startDownload: isSupportRange" + entry.isSupportRange + entry.totalLength);
        if (entry.isSupportRange) {
            startMultiDownload();
        } else {
            startSingleDownload();
    ...
    private void notifyUpdate(DownloadEntry entry, int what) {
        KLog.e("notifyUpdate:" + what + ":" + entry.currentLength);
...
//        }
    ...
    public synchronized void onDownloadError(int index, String message) {
        KLog.e("onDownloadError:" + message);
